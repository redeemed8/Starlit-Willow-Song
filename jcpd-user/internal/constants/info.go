...
func recoverMysql() {
	...
	if db == nil {
		log.Println(Hint(fmt.Sprintf("Application one failed to recover Mysql database , cause by the connection is abnormal , db == nil ...")))
		return
	}
	//	尝试查询
	var version string
	if err1 := db.Raw("Select version()").Scan(&version).Error; err1 != nil || version == "" {
		log.Printf(Hint(fmt.Sprintf("Application one failed to recover Mysql database , cause by the connection is abnormal by test , err = %v ... \n", err1)))
		return
	}
	//	查询到 version , 说明成功恢复
	options.C.DB = db
	...
	//	恢复 MysqlOnce , 等待下次重用
	mysqlOnce = newOnce()
	log.Println(Info(fmt.Sprintf("Application one recover Mysql database Successfully ... ")))
}
...
func recoverRedis() {
	...
	if rdb == nil {
		log.Println(Info(fmt.Sprintf("Application one failed to recover Redis database , cause by the connection is abnormal , rdb == nil ... ")))
		return
	}
	//	测试连接
	_, err := rdb.Ping(context.Background()).Result()
	if err != nil {
		log.Printf(Info(fmt.Sprintf("Application one failed to recover Redis database , cause by the connection is abnormal by test , err = %v ... \n", err)))
		return
	}
	//	查询到 version , 说明成功恢复
	options.C.RDB = rdb
	...
	//	恢复 MysqlOnce , 等待下次重用
	redisOnce = newOnce()
	log.Println(Info(fmt.Sprintf("Application one recover Redis database Successfully ... ")))
